 ...
 *   4. For AuditAndSet: creates a remediation task automatically
...
#   4. Assigns the policy to a scope you choose${isRemediation ? '\n#   5. Creates a remediation task (AuditAndSet mode — triggers actual remediation)' : ''}
#
...
Write-Host "  Mode:    ${isRemediation ? 'AuditAndSet (Remediation)' : 'Audit (Monitor Only)'}" -ForegroundColor ${isRemediation ? 'Yellow' : 'Gray'}
Write-Host ''
...
Write-Host "    \\$assignment = Get-AzPolicyAssignment | Where-Object { \\$_.PolicyDefinitionId -like '*$policyName*' }" -ForegroundColor Gray
Write-Host "    Start-AzPolicyRemediation -Name '$configName-remediation' -PolicyAssignmentId \\$assignment.PolicyAssignmentId" -ForegroundColor Gray
Write-Host ''
Write-Host '  Option C — Direct GC assignment (bypasses policy, immediate remediation):' -ForegroundColor Cyan
Write-Host "    New-GuestConfigurationPolicyAssignment \\\`" -ForegroundColor Gray
Write-Host "        -Name '$configName' \\\`" -ForegroundColor Gray
Write-Host "        -ContentUri \\$sasUri \\\`" -ForegroundColor Gray
Write-Host "        -ContentHash '$hash' \\\`" -ForegroundColor Gray
Write-Host "        -AssignmentType 'ApplyAndAutoCorrect' \\\`" -ForegroundColor Gray
Write-Host "        -VMName '<vm-name>' -ResourceGroupName '<rg-name>'" -ForegroundColor Gray
Write-Host ''
` : ''}
# ─── Summary ─────────────────────────────────────────────────────────────────
...
Write-Host "  Mode:              ${isRemediation ? 'AuditAndSet (Remediation)' : 'Audit (Monitor Only)'}" -ForegroundColor White
...
${isRemediation ? "Write-Host '  Remember: Create a remediation task after assignment to enable remediation!' -ForegroundColor Yellow" : ''}
Write-Host ''
...
${isRemediation ? `Write-Host "     Start-AzPolicyRemediation -Name '${safeName}-remediation' -PolicyAssignmentId \\$assignment.PolicyAssignmentId" -ForegroundColor Gray` : ''}
Write-Host ''
`;
}
